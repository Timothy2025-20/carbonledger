//! Shared test helpers — deploy contracts, create addresses, build strings.

use soroban_sdk::{testutils::Address as _, Address, Env, String};
use carbon_registry::{CarbonRegistryContract, CarbonRegistryContractClient};

/// Convert a `&str` into a Soroban `String`.
pub fn s(env: &Env, v: &str) -> String {
    String::from_str(env, v)
}

/// Deploy and initialise a fresh `carbon_registry` contract.
/// Returns `(client, admin, oracle_addr, verifier_addr)`.
